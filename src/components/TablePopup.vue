    ...
    <table class="min-w-full table-auto">
      <thead>
        <tr class="bg-gray-200">
          <th v-for="col in columns" :key="col" class="editable-heading px-4 py-2 border" contenteditable="true"
            @input="updateHeadingContent">Header {{ col }}</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(row, rowIndex) in rows" :key="rowIndex">
          <td v-for="colIndex in columns" :key="colIndex" class="px-4 py-2 border">
            <input v-model="rows[rowIndex][colIndex]" class="w-full border-none focus:outline-none" />
          </td>
        </tr>
      </tbody>
    </table>
